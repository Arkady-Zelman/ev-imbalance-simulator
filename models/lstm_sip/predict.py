"""
SIP Inference — load trained LSTM and predict the next N settlement periods.

Usage
-----
    from models.lstm_sip.predict import load_artifacts, predict
...
the exact columns listed in `meta['feature_cols']` (MIP already excluded).
"""
...
def load_artifacts(
    weights_path: str | Path = _HERE / "lstm_sip.pth",
    scaler_path:  str | Path = _HERE / "lstm_sip_scaler.pkl",
    device: str = "cpu",
) -> Tuple[LSTM, dict]:
    """
...
def predict(
    ...
    """
    Autoregressively forecast SIP for the next `horizon` settlement periods.
    ...
    -------
    pd.Series
        Forecast SIP values (GBP/MWh) with a 30-min DatetimeIndex.
    """
    scaler     = meta["scaler"]
    ...
    with torch.no_grad():
        for step in range(horizon):
            ...
            # Next row: exogenous features from 24h ago, SIP = prediction
    ...
    return pd.Series(forecast, index=forecast_index, name="sip_forecast_gbp_mwh")
...
if __name__ == "__main__":
    ...
    df = pd.read_parquet(_HERE.parent.parent / "data" / "training_data_snapshot.parquet")
    drop = [c for c in ["mip_gbp_mwh", "wind_fc_mw", "demand_fc_mw"] if c in df.columns]
    df   = df.drop(columns=drop).dropna()

    forecast = predict(df, model, meta, horizon=48)
    print("\nNext 24h SIP forecast (GBP/MWh):")
